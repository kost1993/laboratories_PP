...
int main (int argc, char **argv) {
	...
	if (argc == 2) {
		...
		do {
			...
			data_read_bytes = read(inp_file_d, buffer, size_of_read_for_hash);
			numberhash[index_tread] = bytestoint(buffer);
			//printf("%s\n", buffer);
			printf("%d\t%llx\n", index_tread, numberhash[index_tread]);
			if(index_tread < NUM_THREADS) {
				
				if(pthread_create(&id[index_tread], NULL , work, &numberhash[index_tread])) {
